...
public interface SerieRepository extends JpaRepository<Serie, Long> {
    ...
    List<Serie> findByTotalTemporadaLessThanEqualAndAvaliacaoGreaterThanEqual(int temporadas,double avaliacao);

    @Query("select s from Serie s WHERE s.totalTemporada <= :temporadas AND s.avaliacao >= :avaliacao")
    List<Serie> seriePorTemporadasEAvaliacao(int temporadas, double avaliacao);

    @Query("SELECT e FROM Serie s JOIN s.episodios e WHERE e.titulo ILIKE %:trechoEpisodio%")
    List<Episodio> episodioPorTrecho(String trechoEpisodio);

    @Query("SELECT e FROM Serie s JOIN s.episodios e WHERE s = :serie ORDER BY e.avaliacao DESC LIMIT 5")
    List<Episodio> buscarTopEspisodios(Serie serie);
